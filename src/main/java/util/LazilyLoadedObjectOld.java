...
public class LazilyLoadedObjectOld<T> {
    public interface Copier<T> {T copy(final T src);}
    public interface Instantiator<T> {T create();}
    public interface Mutator<T> {void mutate(final T value);}
    
    private T value = null;
    private boolean lazy = true;
    private LazilyLoadedObjectOld<T> previous;
    private Copier<T> copier;
    private Instantiator<T> instantiator;
    
    public LazilyLoadedObjectOld(final LazilyLoadedObjectOld<T> previous,
    ...
    private void cpy() {
        if(previous != null) value = copier.copy(previous.read());
        else init();
    }
